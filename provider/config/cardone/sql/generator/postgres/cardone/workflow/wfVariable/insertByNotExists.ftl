...
<#if (insert_wfVariableId??) && (insert_wfVariableId_value??)>
${prefixName?string('  ', ', ')}:insert_wfVariableId_value
<#assign prefixName = false>
</#if>
WHERE NOT EXISTS (SELECT 1 FROM "c1_wf_variable" E
<#assign prefixName = true>
...
${prefixName?string('WHERE ', 'AND ')}E."wf_variable_id" = :where_and_eq_wfVariableId_value
<#else>
${prefixName?string('WHERE ', 'AND ')}(E."wf_variable_id" IS NULL OR E."wf_variable_id" = '')
</#if>
<#assign prefixName = false>
</#if>
))